...
void get_rhs(double t, int nx, double f[nx], double x[nx], double rhs[nx], int nghost)
{
  ...
  //
  #pragma omp parallel for default(none) shared(rhs, der2f, nx) schedule(static)
  for (int i = 0; i < nx; i++)
  {
    rhs[i] = 0.001 * der2f[i];
  }
...
void RuKu3(double t, double dt, int nx, double y0[nx], double x[nx], int nghost)
{
  ...
  #pragma omp parallel for default(none) shared(nx, dt, rhs, y0, y1, k1) schedule(static)
  for (int i = 0; i < nx; i++)
  {
    k1[i] = rhs[i] * dt;
    y1[i] = y0[i] + k1[i] * 8 / 15;
  ...
  #pragma omp parallel for default(none) shared(nx, dt, rhs, y0, y2, k1, k2) schedule(static)
  for (int i = 0; i < nx; i++)
  {
    k2[i] = rhs[i] * dt;
  ...
  #pragma omp parallel for default(none) shared(nx, dt, rhs, y0, k1, k3) schedule(static)
  for (int i = 0; i < nx; i++)
  {
    k3[i] = rhs[i] * dt;
...
void initcond(int nx, double y0[nx], double x[nx])
{
  ...
  double x0 = 0.;
  //
  #pragma omp parallel for default(none) shared(nx, x, x0, y0, ampl, width) schedule(static)
  for (int i = 0; i < nx; i++)
  {
...
void der2(int nx, double fx[nx], double x[nx], double der2f[nx], int nghost)
{
  ...
  switch (nghost)
  {
    // -----------------------------------
  case 1:
    #pragma omp parallel for default(none) shared(nghost, nx, der2f, fx) schedule(static)
    for (int i = nghost; i < nx - nghost; i++)
    {
      der2f[i] =
          +1. * fx[i - nghost] - 2. * fx[i] + 1. * fx[i + nghost];
    }
    // Periodic boundary
    der2f[0] =
        +1. * fx[nx - 1] - 2. * fx[0] + 1. * fx[nghost];
    der2f[nx - 1] =
        +1. * fx[nx - 2] - 2. * fx[nx - 1] + 1. * fx[0];

    #pragma omp parallel for default(none) shared(der2f, nx, dx) schedule(static)
    for (int i = 0; i < nx; i++)
    {
