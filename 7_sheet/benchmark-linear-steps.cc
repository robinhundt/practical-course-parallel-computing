  ...
  for (int i=0; i<numberOfTests; i++) {
    ...
    if (rank == 0) {
        ...
        for(int k=0; k<nloop;k++){
            MPI_Ssend( buf, bufSize, MPI_DOUBLE, 1, k, MPI_COMM_WORLD );
            MPI_Recv( buf, bufSize, MPI_DOUBLE, 1, k, MPI_COMM_WORLD, 
                    &status ); 
        }
        t2 = MPI_Wtime();
        ...
        for(int j =0; j<nloop;j++){
            MPI_Recv( buf, bufSize, MPI_DOUBLE, 0, j, MPI_COMM_WORLD, &status );
            MPI_Ssend( buf, bufSize, MPI_DOUBLE, 0, j, MPI_COMM_WORLD );
        }
